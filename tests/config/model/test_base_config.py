...
def test_deep_update_flat_dict_override():
    ...
    assert result == {'a': 0, 'b': 2, 'c': 3}, 'Dictionaries should be a merge of the originals'
...
def test_deep_update_add_nested_dict():
    ...
    assert result == {'a': 0, 'b': {'c': 4}, 'c': 3}, 'Dictionaries should be a merge of the originals'
...
def test_deep_update_update_nested_dict():
    ...
    assert result == {'a': 1, 'b': {'c': 4}}, 'Dictionaries should be a merge of the originals'
...
def test_deep_update_update_nested_dict_new_keys():
    ...
    assert result == {'a': 1, 'b': {'c': 1, 'd': 4, 'e': 5}}, 'Dictionaries should be a merge of the originals'
...
def test_parse_bases_ref_base_same_file():
    ...
    assert result == {'a': 4, 'b': 2, 'c': 3}, 'Return should be a merge of two documents'
...
def test_parse_bases_path_base_same_file():
    ...
    result = mod.parse_bases({'bases': [{'path': 'project.yaml', 'index': 1}], 'c': 3, 'a': 4})
    assert result == {'a': 4, 'b': 2, 'c': 3}, 'Return should be a merge of two documents'
...
def test_parse_bases_invalid_path_reference():
    ...
    with raises(ValueError) as exc:
        _ = mod.parse_bases({'bases': [{'path': 'missing.yaml', 'index': 1}], 'c': 3, 'a': 4})
    ...
    assert isinstance(exc.value.args[1], dict), 'Exception should return additional information about the error'
