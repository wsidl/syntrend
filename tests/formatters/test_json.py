...
def test_single_object(project, monkeypatch):
    ...
    assert (
        output[0] == '{"f1": "string", "f2": 10}'
    ), 'Should generate an encoded json string of multiple values'
...
def test_multiple_objects(project, monkeypatch):
    ...
    assert (
        len(output) == 3
    ), 'Should generate 3 lines of json output with an extra empty line'
...
def test_multiple_objects_as_collection(project, monkeypatch):
    ...
    assert (
        output[1] == '  {"f1": "string", "f2": 10},'
    ), 'Should generate an indented json string of multiple values'
    assert (
        len(output) == 5
