...
def test_static_ref_events(project_result):
    ...
    assert all(
        [timestamps[a - 1] + 5 == timestamps[a] for a in range(1, len(timestamps))]
    )
...
def test_num_trend(project_result):
    assert project_result.exit_code == 0, 'Command should complete successfully'
    generated_lines = [int(line) for line in project_result.output[:-1].split('\n')]
    assert all(
        [
            generated_lines[i + 1] - 1 == generated_lines[i]
            for i in range(len(generated_lines) - 1)
        ]
...
def test_sine_wave(project_result):
    ...
    assert project_result.exit_code == 0, 'Command should complete successfully'
    generated_lines = [float(line) for line in project_result.output[:-1].split('\n')]
    new_sine_values = [
